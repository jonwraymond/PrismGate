...
mod tests {
    ...
    async fn test_multi_client_connect_disconnect() {
        ...
        for _ in 0..5 {
            ...
            assert_eq!(tools.len(), 13, "each client should see 13 meta-tools");
    ...
    async fn test_session_counter_accuracy() {
        ...
        assert_eq!(peer1.list_all_tools().await.unwrap().len(), 13);
        assert_eq!(peer2.list_all_tools().await.unwrap().len(), 13);
        assert_eq!(peer3.list_all_tools().await.unwrap().len(), 13);
