...
## Quick Wins
- [x] 1. Wire telemetry: call `record_bytes` from `call_tool_chain` pipeline and expose via `prismgate://stats`
...
- [x] 4. Session purge command: `prismgate purge` + optional MCP tool
...
- [x] 10. Host skill `skills/prismgate-context/SKILL.md` — compact/resume must call `session_search(card=true)`
...
- [ ] D. Docs update: README + `llms.txt` + `prismgate://call_tool_chain` guide
